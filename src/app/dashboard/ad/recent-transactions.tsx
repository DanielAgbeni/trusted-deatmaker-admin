...
export default function RecentTransactions() {
  const { data: withdrawalsData, isLoading: wLoading } = useGetWithdrawalsQuery({ page: 0, size: 5 });
  const { data: depositsData, isLoading: dLoading } = useGetDepositsQuery({ page: 0, size: 5 });

  const withdrawals = withdrawalsData?.data?.content || [];
  const deposits = depositsData?.data?.content || [];

  const mapToWalletTransaction = (t: AdminTransaction, type: 'withdrawal' | 'deposit'): WalletTransaction => ({
    id: t.transactionId,
    user: {
